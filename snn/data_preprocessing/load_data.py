...
def get_example(hdf5_group, idx, T=80, classes=[], size=[1, 26, 26], dt=1000, x_max=1, polarity=True):
    ...
    return torch.FloatTensor(data.T), make_output_from_labels(label, T, classes, size)
...
def chunk_evs_pol(times, addrs, deltat=1000, size=[2, 304, 240], x_max=1, polarity=True):
    ...
    chunks = np.zeros([len(ts)]+size, dtype='float')
    idx_start = 0
    idx_end = 0
