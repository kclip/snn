...
def get_acc_and_loss(network, hdf5_group, test_indices, T, classes, input_shape, dt, x_max, polarity):
    ...
    for j, idx in enumerate(test_indices):
        ...
        true_classes = torch.cat((true_classes, lbl))
...
def get_acc_loss_and_spikes(network, hdf5_group, test_indices, T, classes, input_shape, dt, x_max, polarity):
    """"
    Compute loss and accuracy on the indices from the dataset precised as arguments
    """
    network.eval()
...
def test(network, j, train_data, train_indices, test_data, test_indices, T, classes, input_shape,
    ...
    if (j + 1) % test_period == 0:
        if (test_accs is not None) or (test_losses is not None):
            test_acc, test_loss = get_acc_and_loss(network, test_data, test_indices, T, classes, input_shape, dt, x_max, polarity)
        ...
        if (train_accs is not None) or (train_losses is not None):
            train_acc, train_loss = get_acc_and_loss(network, train_data, train_indices, T, classes, input_shape, dt, x_max, polarity)
